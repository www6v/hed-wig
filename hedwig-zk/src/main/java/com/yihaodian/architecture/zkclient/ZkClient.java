...
import com.yihaodian.architecture.zkclient.ZkEventThread.ZkEvent;
...
import com.yihaodian.architecture.zkclient.serialize.ZkSerializer;
import com.yihaodian.architecture.zkclient.util.ZkPathUtil;
...
public class ZkClient implements Watcher {
    ...
    private KeeperState _currentState = KeeperState.Unknown;
     ...
     *             if any other exception occurs
     */
    public void createPersistent(String path, boolean createParents) throws ZkInterruptedException, IllegalArgumentException, ZkException,
            RuntimeException {
        try {
     ...
     *             if any other exception occurs
     */
    public void createPersistent(String path, Object data) throws ZkInterruptedException, IllegalArgumentException, ZkException,
            RuntimeException {
     ...
     *             if any other exception occurs
     */
    public String createPersistentSequential(String path, Object data) throws ZkInterruptedException, IllegalArgumentException,
     ...
     *             if any other exception occurs
     */
    public String create(final String path, Object data, final CreateMode mode) throws ZkInterruptedException, IllegalArgumentException,
            ZkException, RuntimeException {
        if (path == null) {
     ...
     *             if any other exception occurs
     */
    public void createEphemeral(final String path, final Object data) throws ZkInterruptedException, IllegalArgumentException, ZkException,
            RuntimeException {
     ...
     *             if any other exception occurs
     */
    public String createEphemeralSequential(final String path, final Object data) throws ZkInterruptedException, IllegalArgumentException,
            ZkException, RuntimeException {
    ...
    protected Stat existsWithStat(final String path, final boolean watch) {
        return retryUntilConnected(new Callable<Stat>() {
            @Override
            public Stat call() throws Exception {
                return _connection.existsWithStat(path, watch);
            }
        });
    }
    ...
    public Stat existsWithStat(final String path) {
        return existsWithStat(path, hasListeners(path));
    }
            ...
            _eventThread.send(new ZkEvent("Data of " + path + " changed sent to " + listener) {
                ...
                public void run() throws Exception {
                    ...
                    try {
                        ...
                        LOG.info("Can't deserialize object by java deserializer , use raw data directly!!!", error);
                        // 兼容其它客户端写入非对象序列化的数据
                        byte[] data = null;
                        try {
                            data = readRawData(path, true);
                        } catch (Throwable t) {
                            LOG.error("read raw data error within fireDataChangedEvents", t);
                        }
                        listener.handleDataChange(path, data);
                    } catch (ZkNoNodeException e) {
    ...
    public boolean waitForKeeperState(KeeperState keeperState, long time, TimeUnit timeUnit) throws ZkInterruptedException {
        ...
        try {
            ...
            while (!_currentState.equals(keeperState)) {
                if (!stillWaiting) {
                    return false;
                }
     ...
     */
    public <T> T retryUntilConnected(Callable<T> callable) throws ZkInterruptedException, IllegalArgumentException, ZkException,
            RuntimeException {
        if (_zookeeperEventThread != null && Thread.currentThread() == _zookeeperEventThread) {
            throw new IllegalArgumentException("Must not be done in the zookeeper event thread.");
        }
    ...
    public Stat writeRawData(final String path, final byte[] data) {
        final int expectedVersion = -1;
        return writeRawData(path, data, expectedVersion);
    }
    ...
    public Stat writeRawData(final String path, final byte[] data, final int expectedVersion) {
        return (Stat) retryUntilConnected(new Callable<Object>() {
     ...
     *             if the connection timed out due to thread interruption
     */
    public void connect(final long maxMsToWaitUntilConnected, Watcher watcher) throws ZkInterruptedException, ZkTimeoutException,
