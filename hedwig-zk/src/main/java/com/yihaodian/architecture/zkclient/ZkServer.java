...
public class ZkServer {
	...
	private void startSingleZkServer(final int tickTime, final File dataDir, final File dataLogDir, final int port) {
		try {
			...
			/*
			 * incompatible api when upgrade apache zookeeper client from 3.3.5
			 * to 3.4.7-SNAPSHOT.
			 */
			// _nioFactory = new NIOServerCnxnFactory(new
			// InetSocketAddress(port));
			_nioFactory = new NIOServerCnxnFactory();
			_nioFactory.configure(new InetSocketAddress(port), Integer.MAX_VALUE);
