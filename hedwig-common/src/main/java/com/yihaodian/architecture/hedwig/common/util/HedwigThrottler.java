 ...
 */
public class HedwigThrottler {
	private static final Logger LOG = LoggerFactory.getLogger(HedwigThrottler.class);
	 ...
	 */
	public HedwigThrottler(long bandwidthPerSec) {
	 ...
	 *            bandwidth allowed in bytes per second.
	 */
	public HedwigThrottler(long period, long bandwidthPerSec) {
	 ...
	 * make the current thread sleep if I/O rate is too fast compared to the
	 * given bandwidth.
	 * 
	 * @param numOfBytes
	 *            number of bytes sent/received since last time throttle was
	 *            called
	 */
		...
		while (curReserve <= 0) {
			...
			if (now < curPeriodEnd) {
				if (LOG.isDebugEnabled()) {
					String tst = DateUtil.getDateTime(curPeriodStart);
					String tet = DateUtil.getDateTime(now);
					LOG.debug("from [" + tst + "] to [" + tet + "] time period[" + (now - this.curPeriodStart) + "]ms send content is ["
							+ this.bytesAlreadyUsed + "] bytes,config period[" + this.period + "]ms stream data is [" + this.getBandwidth()
							+ "],will block [" + (curPeriodEnd - now) + "]ms");
				}
				// Wait for next period so that curReserve can be increased.
				try {
					wait(curPeriodEnd - now);
				} catch (InterruptedException e) {
					// Abort throttle and reset interrupted status to make sure
					// other
